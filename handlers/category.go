...
import (
	filmsdto "dumbmerch/dto/category"
...
func (h *CategoryHandler) CreateCategory(c echo.Context) error {
	request := new(filmsdto.CreateCategoryRequest)
	...
	data, err := h.CategoryRepository.CreateCategory(name)
	if err != nil {
...
func (h *CategoryHandler) UpdateCategory(c echo.Context) error {
	request := new(filmsdto.UpdateCategoryRequest)
	if err := c.Bind(&request); err != nil {
		return c.JSON(http.StatusBadRequest, dto.ErrorResult{Code: http.StatusBadRequest, Message: err.Error()})
	}
...
func convertResponseCategory(u models.Category) filmsdto.CategoryResponse {
	return filmsdto.CategoryResponse{
		ID:   u.ID,
		Name: u.Name,
	}
}
